...
namespace TowerDefense.UI
{
	...
	public class EndGameScreen : MonoBehaviour
	{
		...
		/// </summary>
		protected void OpenEndGameScreen(string endResultText, string result)
		{
			...
			if (level != null) 
			{
                if (Application.systemLanguage.Equals(SystemLanguage.Spanish)
                    & !string.IsNullOrEmpty(level.spanishName))
                {
                    endGameMessageText.text = string.Format(endResultText, level.spanishName.ToUpper(), 
                        result == "victory"?"COMPLETADO": "FALLIDO");
                }
                else
                {
				    endGameMessageText.text = string.Format (endResultText, level.name.ToUpper (),
                        result == "victory"?"COMPLETED!":"FAILED!");
                }
		...
		{
			OpenEndGameScreen(levelCompleteText, "victory");
		...
		{
			OpenEndGameScreen(levelFailedText, "defeat");
